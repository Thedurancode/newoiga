...
const app = new Hono2();
const CreateVenueSchema = objectType({
  name: stringType().min(1),
  description: stringType().optional(),
  address: stringType().optional(),
  city: stringType().optional(),
  image_url: stringType().url().optional(),
  logo_url: stringType().url().optional(),
  website: stringType().url().optional(),
  phone: stringType().optional()
});
const UpdateVenueSchema = CreateVenueSchema.partial().extend({
  id: numberType().int().positive().optional()
});
const CreateEventSchema = objectType({
  title: stringType().min(1),
  description: stringType().optional(),
  venue_id: numberType().int().positive(),
  start_date_time: stringType(),
  end_date_time: stringType().optional(),
  price: numberType().min(0).optional().nullable(),
  image_url: stringType().optional(),
  image_file: anyType().optional(),
  ticket_url: stringType().optional(),
  is_featured: numberType().int().min(0).max(1).default(0),
  is_special: numberType().int().min(0).max(1).default(0)
});
const UpdateEventSchema = objectType({
  title: stringType().min(1).optional(),
  description: stringType().optional(),
  venue_id: numberType().int().positive().optional(),
  start_date_time: stringType().optional(),
  end_date_time: stringType().optional(),
  price: numberType().min(0).optional().nullable(),
  image_url: stringType().optional(),
  image_file: anyType().optional(),
  ticket_url: stringType().optional(),
  is_featured: numberType().int().min(0).max(1).optional(),
  is_special: numberType().int().min(0).max(1).optional()
});
...
app.get("*", serveStatic({
  ...
  getContent: async (path, context) => {
    return context.env.ASSETS.fetch(path);
  }
}));
export {
  app as default
